...
export function AgencyClient({ language }: { language: Language }) {
  ...
  function findLocation() {
    if (location) { setLocation(null); setError(""); return; }
    ...
    <section className="content-width agency-search">
      <div className="agency-toolbar">
        <div className="agency-filters" role="group" aria-label="Service type">
          ...
          <button className={languageOnly ? "active" : ""} onClick={() => setLanguageOnly((value) => !value)}>🌐 {t.languageOnly}</button>
        </div>
        <button className={`location-button ${location ? "active" : ""}`} onClick={findLocation} disabled={locating}>📍 {locating ? t.locating : location ? t.reset : t.location}</button>
      </div>
      {location && <p className="location-note">{t.approximate}</p>}
      ...
      <div className="agency-grid">
        {agencies.map((agency) => <article className="agency-card" key={agency.id}>
          ...
          <dl><div><dt>{t.hours}</dt><dd>{localized(agency.hours, language)}</dd></div><div><dt>{t.language}</dt><dd>{agency.supported_languages.includes(language) ? "✓" : "—"}</dd></div><div><dt>{t.verified}</dt><dd>{agency.verified_at}</dd></div></dl>
