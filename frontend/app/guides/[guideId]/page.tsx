...
export default async function GuideDetailPage({ params, searchParams }: { params: Promise<{ guideId: string }>; searchParams: Promise<{ lang?: string }> }) {
  ...
  return <main><Header language={language} /><div className="detail-layout content-width"><Link href={withLanguage("/guides", language)} className="back-link">{t.back}</Link><header className="detail-header"><span className={`category-badge ${guide.category}`}>{guide.category === "residency" ? t.residency : t.labor}</span><h1>{localized(guide.title, language)}</h1><p>{localized(guide.summary, language)}</p></header><div className="detail-columns"><div className="detail-main"><section className="detail-card"><h2>{t.steps}</h2><ol className="steps">{steps.map((step, index) => <li key={step}><span>{index + 1}</span><p>{step}</p></li>)}</ol></section><section className="detail-card"><h2>{t.documents}</h2><ul className="check-list">{documents.map((item) => <li key={item}>✓ {item}</li>)}</ul></section><section className="detail-card caution"><h2>{t.caution}</h2><ul>{cautions.map((item) => <li key={item}>{item}</li>)}</ul></section></div><aside><section className="detail-card"><h2>{t.help}</h2>{agencies.map((agency) => <div className="agency" key={agency.id}><h3>{localized(agency.name, language)}</h3><p>{localized(agency.description, language)}</p><a href={`tel:${agency.phone}`}>☎ {agency.phone}</a><a href={agency.website} target="_blank" rel="noreferrer">{t.website}</a></div>)}</section><section className="source-card"><span>{t.source}</span><a href={guide.source_url} target="_blank" rel="noreferrer">{localized(guide.source_name, language)} ↗</a><small>{t.verified} {guide.verified_at}</small></section></aside></div></div></main>;
}
